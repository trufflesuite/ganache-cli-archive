  ...
  this.keyExists(key, function(err, exists) {
    ...
    if (exists) {
      ...
      if (self.address == null) {
        ...
        self.web3.eth.getStorageAt(to.hex(self.address), to.hex(key), self.fallback_block_number, function(err, value) {
          if (err) return callback(err);
...
FallbackStorageTrie.prototype.fetchAccount = function(address, callback) {
  ...
  var self = this;
  this.web3.eth.getCode(address, self.fallback_block_number, function(err, code) {
    if (err) return callback(new Error("Error communicating with fallback provider: " + err.message));

    self.web3.eth.getBalance(address, self.fallback_block_number, function(err, balance) {
      if (err) return callback(err);

      self.web3.eth.getTransactionCount(address, self.fallback_block_number, function(err, nonce) {
        if (err) return callback(err);
