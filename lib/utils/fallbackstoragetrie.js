...
FallbackStorageTrie.prototype.put = function(key, value, callback) {
  console.log("PUT ON FALLBACK TRIE: " + key.toString("hex"), value.toString("hex"));
  MerklePatriciaTree.prototype.put.call(this, key, value, callback);
};
...
FallbackStorageTrie.prototype.get = function(key, callback) {
  ...
  this.keyExists(key, function(err, exists) {
    ...
    if (exists || !self.fallback) {
      console.log("GETTING STORAGE FROM TRIE: " + self.address, to.hex(key));
      //self.blockchain.getDirectStorage(self.address, key, "latest", callback);
      MerklePatriciaTree.prototype.get.call(self, utils.toBuffer(key), function(err, value) {
        if (err) return callback(err);
        console.log("RESULT FROM STORAGE:", err, value)
        callback(null, value)
      });
    } else {
      console.log("GETTING STORAGE OFF THE WIRE: " + self.address, to.hex(key));
      ...
      if (self.address == null) {
        ...
        self.web3.eth.getStorageAt(to.hex(self.address), to.hex(key), "latest", function(err, value) {
          ...
          callback(null, utils.toBuffer(value));
