...
LevelUpArrayAdapter.prototype.get = function(index, callback) {
  ...
  this.length(function(err, length) {
    if (err) return callback(err);
    if (index > length) {
      return callback(new Error("ArrayAdapter out of range: " + index + "; length: " + length));
    }
...
LevelUpArrayAdapter.prototype.pop = function(val, callback) {
  var self = this;

  this.length(function(err, result) {
    if (err) return callback(err);

    // TODO: Do this in atomic batch.
    self.db._put(length + "", val, function(err) {
      if (err) return callback(err);
      self.db.put("length", length + 1, callback);
    });
  });
