...
GethApiDouble.prototype.eth_getTransactionReceipt = function(hash, callback) {
  var tx_result = this.state.getTransaction(hash);
  var result = null;

  if (tx_result !== undefined) {
    result = {
      transactionHash: hash,
      transactionIndex: to.hex(utils.intToHex(0)),
      blockHash: to.hex(tx_result.block.hash()),
      blockNumber: to.hex(tx_result.block_number),
      cumulativeGasUsed: to.hex(tx_result.gasUsed),  // TODO: What should this be?
      gasUsed: to.hex(tx_result.gasUsed),
      contractAddress: tx_result.createdAddress,
      logs: tx_result.logs.map(function(log) {return log.toJSON()})
    };
  }
...
GethApiDouble.prototype.eth_getTransactionByHash = function(hash, callback) {
  ...
  if (tx_result !== undefined) {
    result = txhelper.toJSON(tx_result.tx, tx_result.block)
  }
...
GethApiDouble.prototype.eth_getTransactionCount = function(address, block_number, callback) {
  this.state.getTransactionCount(address, callback);
}
...
GethApiDouble.prototype.eth_getLogs = function(filter, callback) {
  var logs = this.state.getLogs(filter);
  callback(null, logs);
};
