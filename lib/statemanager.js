...
var Transaction = require('ethereumjs-tx/fake.js');
...
Transaction.prototype._getSenderAddress = 
...
StateManager.prototype.queueRawTransaction = function(rawTx, callback) {
  ...
  var txParams = {
    ...
    v:     (tx.v && tx.v.length    ) ? '0x'+tx.v.toString('hex')     : null,
    r:     (tx.r && tx.r.length    ) ? '0x'+tx.r.toString('hex')     : null,
    s:     (tx.s && tx.s.length    ) ? '0x'+tx.s.toString('hex')     : null,
  }
...
StateManager.prototype.queueTransaction = function(method, tx_params, callback) {
  ...
  if (tx_params.from != null) {
    rawTx.from = utils.addHexPrefix(tx_params.from);
  }
  ...
  this.blockchain.getQueuedNonce(address, function(err, nonce) {
    ...
    // if( to.hex(address) in self.accounts) {
    //   var secretKey = self.accounts[to.hex(address)].secretKey;
    //   tx.sign(secretKey);
    // }
...
// =======
  ...
  self.blockchain.latestBlock((err, block) => {
    self.blockchain.getTransactionReceipt(tx_hash, function(err, receipt) {
      if (err) return callback(err);
      // receipt = typeof receipt === "object" ? receipt: receipt.toJSON();
...
StateManager.prototype.processTransaction = function(from, tx, callback) {
  ...
  var tx_hash = to.hex(tx.hash(true));
...
StateManager.prototype.getLogs = function(filter, callback) {
  ...
  if( this.blockchain instanceof ForkedBlockchain &&
    fromBlock < this.blockchain.getEffectiveBlockNumber(this.blockchain.fork_block_number)) {
...
StateManager.prototype.revert = function(snapshot_id, callback) {
  ...
  while (self.snapshots.length > snapshot_id) {
    ...
    async.whilst(
      () => {
        return self.blockchain.getHeight() > snapshot.blockNumber
      },
      (cb) => {
        self.blockchain.popBlock(cb);
      },
      () => {
        callback(null, true);
      })
