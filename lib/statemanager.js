...
StateManager.prototype.processTransaction = function(from, tx, callback) {
  ...
  this.blockchain.processNextBlock(function(err, results) {
    ...
    self.blockchain.getBlockLogs(block.header.number, function(err, logs) {
      if (err) return callback(err);
...
StateManager.prototype.getLogs = function(filter, callback) {
  ...
  var current = utils.bufferToInt(fromblock.header.number);
  var last = utils.bufferToInt(toblock.header.number);

  async.whilst(function() {
    return current <= utils.bufferToInt(toblock.header.number);
  }, function(finished) {
    var hexnumber = to.hex(current);

    self.blockchain.getBlockLogs(hexnumber, function(err, blockLogs) {
      if (err) return finished(err);
      logs.push.apply(logs, blockLogs);
...
StateManager.prototype.snapshot = function() {
  this.snapshots.push({
    ...
    blockNumber: this.blockchain.getHeight()
  });
