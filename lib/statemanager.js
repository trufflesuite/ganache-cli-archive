...
StateManager = function(options) {
  ...
  this.net_version = options.network_id || new Date().getTime();
...
StateManager.prototype.snapshot = function(callback) {
  ...
  this.blockchain.snapshot(function(err) {
    if (err) return callback(err);

    self.snapshots.push({
      root: self.stateTrie.root,
...
StateManager.prototype.revert = function(snapshot_id, callback) {
  ...
  async.whilst(function() {
    ...
    //self.stateTrie.root = snapshot.root;
    self.blockchain.revert(snapshot.blockNumber, finished);
  }, function(err) {
    if (err) return callback(err);
    callback(null, true)
  });
