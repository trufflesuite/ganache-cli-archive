...
var Interface = {
  server: function(options) {
    ...
    // One has to close the provider as well now
    var _close = server.close
    server.close = function () {
      var args = Array.prototype.slice.call(arguments);
      provider.close(function () {
        _close.apply(server, args);
      });
    }
  ...
  provider: function(options) {
    ...
    if('db_path' in options) {
      options.db = levelup(options.db_path);
    } else {
      options.db = levelup("/"+Math.random().toString(), {
        db: memdown
      });
    }
    ...
    return {
      ...
      close: function (cb) {
        // can lead to a corrupt state
        options.db.close((err, res) => {
          console.log(err, res);
          cb();
        });
        engine.stop();
