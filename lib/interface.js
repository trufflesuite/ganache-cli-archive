...
var Interface = {
  ...
  webSocketServer: function(options) {
    ...
    if (options.webSocketServer == null) {
      options.webSocketServer = {}
    }

    if (options.webSocketServer.port == null) {
      options.webSocketServer.port = 80;
    }

    var logger = options.logger;
    var provider = this.provider(options);
    var webSocketServer = new WebSocket.Server(options.webSocketServer);
    webSocketServer.on('connection', (webSocket) => {
      webSocket.on('message', (messageJson) => {
        var message = JSON.parse(messageJson);
        switch (message.method) {
          case "eth_subscribe":
            // TODO: add support for pub-sub, at the moment this just returns a token that will never get any notifications
