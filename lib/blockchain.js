...
Blockchain.prototype.processNextAction = function(override) {
  ...
  if (queued.method == "eth_getStorageAt") {
    ...
    this.estimateGas(queued.from, queued.rawTx, intermediary);
  } else {
  ...
  this.stateTrie.get(address, function(err, val) {
    ...
    self.vm.runTx({
      ...
      if (results.vm.exception != 1) {
        callback(new Error("VM Exception while executing call: " + results.vm.exceptionError));
        return;
      }
...
Blockchain.prototype.estimateGas = function(from, rawTx, callback) {
  var self = this;
  ...
  this.stateTrie.get(address, function(err, val) {
    ...
    self.vm.runTx({
      ...
      callback(null, utils.bufferToInt(self.toHex(results.gasUsed)));
    });
  });
};
