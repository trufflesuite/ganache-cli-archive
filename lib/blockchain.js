...
Blockchain.prototype.accountAddresses = function() {
  ...
  for(var i=0; i<this.accounts.length; i++) {
    addresses.push("0x" + this.accounts[i].address);
  }
  return addresses;
}
...
Blockchain.prototype.balanceOf = function(_address) {
  var address = utils.stripHexPrefix(_address);
  var account = this.accounts.filter(function(a) { return a.address == address })[0];
