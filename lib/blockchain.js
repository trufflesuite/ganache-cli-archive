...
Blockchain.prototype.queueEstimateGas = function(tx_params, callback) {
  this.queueAction("eth_estimateGas", tx_params, callback);
};
...
Blockchain.prototype.processNextAction = function(override) {
  ...
  } else if (queued.method == "eth_sendTransaction") {
    this.processTransaction(queued.from, queued.rawTx, intermediary);
  } else if (queued.method == "eth_estimateGas"){
    this.processEstimateGas(queued.from, queued.rawTx, intermediary);
  } else {
    this.processCall(queued.from, queued.rawTx, intermediary);
  }
};

// Transactions called through this method do not save state on the blockchain.
Blockchain.prototype.runTx = function(method, from, rawTx, callback) {
  var self = this;
  ...
  this.stateTrie.get(address, function(err, val) {
    ...
    self.vm.runTx({
      ...
      if (results.vm.exception != 1) {
        callback(new Error("VM Exception while executing " + method + ": " + results.vm.exceptionError));
        return;
      }
...
Blockchain.prototype.processCall = function(from, rawTx, callback) {
  var self = this;
  this.runTx("eth_call", from, rawTx, function(err, results) {
...
Blockchain.prototype.processEstimateGas = function(from, rawTx, callback) {
  var self = this;
  this.runTx("eth_estimateGas", from, rawTx, function(err, results) {
    if (err) return callback(err);

    callback(null, self.toHex(results.gasUsed));
  });
};
...
Blockchain.prototype.processStorageRequest = function(address, position, number, callback) {
  ...
  // var trie = new Trie(block.header.stateRoot);
