...
// Note: Do not use toHex() when you really mean utils.addHexPrefix().
...
Blockchain.prototype.addAccount = function(opts, callback) {
  ...
  if (opts.secretKey) {
    secretKey = utils.toBuffer(this.toHex(opts.secretKey));
  } else {
  ...
  if (opts.balance) {
    account.balance = this.toHex(opts.balance)
  } else {
...
Blockchain.prototype.blockFromBlockTag = function(tag) {
  ...
  if (tag == "latest" || tag == "pending") {
    ...
    var blockNumber = utils.bufferToInt(utils.toBuffer(this.toHex(tag)));
...
Blockchain.prototype.getLogs = function(filter) {
  ...
  if (filter.fromBlock == null || filter.fromBlock == "latest") {
    ...
    var blockNumber = utils.bufferToInt(utils.toBuffer(this.toHex(filter.fromBlock)));
  ...
  if (filter.toBlock == null || filter.toBlock == "latest") {
    ...
    var blockNumber = utils.bufferToInt(utils.toBuffer(this.toHex(filter.toBlock)));
