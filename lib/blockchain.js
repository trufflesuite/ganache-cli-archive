...
Blockchain.prototype.getBalance = function(address, callback) {
  var self = this;
  address = new Buffer(this.toHex(address));
...
Blockchain.prototype.getCode = function(_address) {
  var address = this.toHex(_address);
...
Blockchain.prototype.getBlockByNumber = function(number) {
  ...
  if (number == "latest" || number == "pending") {
    ...
    block = this.blocks[utils.bufferToInt(number - 1)];
  }
    ...
    self.vm.runBlock({
      ...
      for (var i = 0; i < receipt.logs.length; i++) {
        ...
        logs.push({
          ...
          blockNumber: self.toHex(self.blocks.length - 1),
          address: address,
          data: data,
          topics: topics,
          type: "mined"
      ...
      var tx_result = {
        tx: tx,
        block_number: self.toHex(self.blocks.length - 1),
        block: block,
