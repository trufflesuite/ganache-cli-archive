...
BlockchainDouble.prototype.initialize = function(accounts, genesis_block, callback) {
  var self = this;

  if (typeof genesis_block == "function") {
    callback = genesis_block;
    genesis_block = null;
  }
  ...
  this.data.initialize(function(err) {
    ...
    if (options.trie) {
      self.stateTrie = options.trie;
    } else {
      self.stateTrie = new Trie(self.data.db);
    }
    ...
    self.latestBlock(function(err, block) {
      ...
      // If a genesis block wasn't asked for, and we already have a chain,
      // don't create a new block.
      if (!genesis_block && !!block) {
        //self.logger.log("Warning: Existing blockchain exists. Not creating default accounts.");
        return callback();
      }

      // We have a genesis block, or the chain doesn't exist.
      // Let's add the block.
      function getGenesisBlock(cb) {
        if (genesis_block != null) {
          return cb(null, genesis_block);
        }
        self.createBlock(cb);
      }

      getGenesisBlock(function(err, block) {
        if (err) return callback(err);
...
BlockchainDouble.prototype.getBlock = function(number, callback) {
  ...
  if (number.indexOf("0x") >= 0) {
    ...
    // block hash
    if (hash.length > 40) {
      this.data.blockHashes.get(to.hex(hash), function(err, blockNumber) {
        if (err) return callback(err);
        return self.data.blocks.get(blockNumber, callback);
      });
    } else {
      // Block number
...
BlockchainDouble.prototype.popBlock = function(callback) {
  ...
  this.data.blocks.length(function(err, length) {
    ...
    self.data.blocks.pop(function(err, block) {
      if (err) return callback(err);
...
BlockchainDouble.prototype.getTransactionReceipt = function(hash, callback) {
  ...
  this.data.transactionReceipts.get(hash, callback);
};
...
BlockchainDouble.prototype.getBlockLogs = function(number, callback) {
  ...
  this.data.blockLogs.get(number, callback);
};
