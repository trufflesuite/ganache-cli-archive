...
BlockchainDouble.prototype.initialize = function(accounts, block) {
  ...
  if( !self.blockchain ) {
    createNewGenesis(self.db, {timestamp: Math.floor(+(new Date())/1000) + this.timeAdjustment, gasLimit: this.gasLimit, alloc: alloc} , (err, res) => {
    ...
    // block.header.stateRoot = self.stateTrie.root;
...
BlockchainDouble.prototype.getBlock = function(number, callback){
  if (number == "latest" || number == "pending") {
    ...
    if(typeof number === "string" && number.indexOf("0x") > -1 && number.length > 40) {
      this.blockchain.getBlock(to.hex(number), callback);
    } else {
      this.blockchain.getBlock(parseInt(number), callback);
    }
  }
  // var block;
  //
...
BlockchainDouble.prototype.putBlock = function(block, logs, receipts) {
  ...
  this.db.put('logs' + block.hash().toString('hex'), logs, {
    valueEncoding: 'json'
  }, () => {
...
BlockchainDouble.prototype.getBlockLogs = function(number, callback) {
  ...
  this.getBlock(number, (err, block) => {
    this.db.get("logs"+block.hash().toString('hex'), {valueEncoding: "json"}, (err, logs) => {
      callback(null, logs);
    });
  });
};
