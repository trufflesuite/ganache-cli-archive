...
function BlockchainDouble(options) {
  ...
  var oldPut = this.stateTrie.put.bind(this.stateTrie);

  this.stateTrie.put = function(key, value, callback) {
    console.log("PUTTING ON MAIN TRIE: " + key.toString("hex"), value.toString("hex"));
    oldPut(key, value, callback);
  };
...
BlockchainDouble.prototype.createFallbackStorageTrie = function(address) {
  ...
  console.log("####################### CREATING STORAGE TRIE: " + address);
...
BlockchainDouble.prototype.lookupStorageTrie = function(address, callback) {
  ...
  console.log("Getting storage trie: ", address)
...
BlockchainDouble.prototype.processNextBlock = function(callback) {
  ...
  this.vm.runBlock({
    ...
    console.log(results.results[0].vm.logs);
