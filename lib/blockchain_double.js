...
BlockchainDouble = function(options) {
  ...
  this.vm = new VM(this.stateTrie, {
    getBlock: function(number, callback){
      callback(null, self.blocks[number])
    }
  }, {
...
BlockchainDouble.prototype.getCode = function(address, callback) {
  ...
  this.vm.stateManager.getContractCode( address, function( err, result ) {
    if( err != null ) {
      if( err === '0x0' && this.fallbackEnabled ) {
        ...
        web3Fallback.eth.getCode( address, function( err, result ) {
          ...
          // Put the copy of the contract from the fallback into the local chain
          this.vm.stateManager.putContractCode( address, to.hex(result), function(err) {
            callback( null, to.hex( result ) );
          });
        });
...
BlockchainDouble.prototype.hasContractCode = function(address, callback) {
  this.vm.stateManager.getContractCode( address, function( err, result ) {
    if( err != null ) {
      callback( err, false );
    } else {
