...
BlockchainDouble.prototype.snapshot = function(callback) {
  this.vm.stateManager.checkpoint();
  callback();
};

// Remove blocks from the chain. Used for snapshotting.
BlockchainDouble.prototype.revert = function(number, callback) {
  var self = this;

  this.vm.stateManager.revert(function() {
    var removedBlocks = self.blocks.splice(number + 1);
    var removedLogs = self.blockLogs.splice(number + 1);

    removedBlocks.forEach(function(block) {
      delete self.blockHashes[to.hex(block.hash())];
    });
