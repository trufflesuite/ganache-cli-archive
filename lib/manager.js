...
Manager.prototype.response = function(params, result) {
  if (this.resultOnly) {
    ...
    console.log("response: " + JSON.stringify({"id":params.id,"jsonrpc":"2.0","result":result}));
...
Manager.prototype.request = function(params, cb) {
  var result;
  console.log("===================");
  console.log("==> request: " + (params.method || params[0].method));
  console.log(params);
  switch (params.method || params[0].method) {
    ...
    case 'eth_getCode':
      result = this.response(params, '0x' + this.blockchain.getCode(params.params[0]));
      if (cb !== undefined) cb(null, result);
      return result;
    ...
    case 'eth_compileSolidity':
      ...
      fs.writeFile("/tmp/solCompiler.sol", params.params[0], function() {
        result = compiler.compile_solidity("/tmp/solCompiler.sol");
        cb(null, result);
      });
      return;
    default:
