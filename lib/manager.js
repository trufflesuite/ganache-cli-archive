...
Manager.prototype.eth_sendRawTransaction = function(rawTx, callback) {
  ...
  var txParams = {
    from:     tx.from     && '0x'+tx.from.toString('hex'),
    to:       tx.to       && '0x'+tx.to.toString('hex'),
    gas:      tx.gas      && '0x'+tx.gas.toString('hex'),
    gasPrice: tx.gasPrice && '0x'+tx.gasPrice.toString('hex'),
    value:    tx.value    && '0x'+tx.value.toString('hex'),
    data:     tx.data     && '0x'+tx.data.toString('hex'),
  }
  this.eth_sendTransaction(tx, callback);
};
